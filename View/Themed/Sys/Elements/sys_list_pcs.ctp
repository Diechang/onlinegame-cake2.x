...
<?php foreach($pcs as $key => $pc):?>
		<tr>
			<td class="tCenter" nowrap="nowrap">
				<?php echo $this->Html->link("編集", array("controller" => "pcs", "action" => "edit", $pc["Pc"]["id"]), array("class" => "btn"))?>
				<?php echo $this->Form->hidden("Pc." . $key . ".id", array("value" => $pc["Pc"]["id"]))?>
				<?php echo $this->Form->hidden("Pc." . $key . ".title_id", array("value" => $pc["Pc"]["title_id"]))?>
			</td>
			<td class="tCenter">
				<?php echo $this->Form->checkbox("Pc." . $key . ".public", array("checked" => (!empty($pc["Pc"]["public"]))))?>
			</td>
			<td class="tCenter">
				<?php echo $this->Form->checkbox("Pc." . $key . ".pickup", array("checked" => (!empty($pc["Pc"]["pickup"]))))?>
			</td>
			<td class="tCenter">
				<?php echo $this->Form->checkbox("Pc." . $key . ".ad_noredirect", array("checked" => (!empty($package["Pc"]["ad_noredirect"]))))?>
			</td>
			...
			<td class="tCenter" nowrap="nowrap"><?php echo $this->Html->link("<i class='icon-repeat icon-white'></i> 複製", array("controller" => "pcs", "action" => "copy", $pc["Pc"]["id"]), array("class" => "btn btn-info btn-small", "escape" => false), $pc["Pc"]["id"] . " を複製しますか?")?></td>
			<td class="tCenter" nowrap="nowrap"><?php echo $this->Html->link("<i class='icon-remove icon-white'></i> 削除", array("controller" => "pcs", "action" => "delete", $pc["Pc"]["id"]), array("class" => "btn btn-danger btn-small", "escape" => false), $pc["Pc"]["id"] . " を削除しますか?")?></td>
		</tr>
<?php endforeach;?>
	</tbody>
</table>