...
<?php foreach($votes as $key => $v):?>
		<tr class="title_id_<?php echo $v["Vote"]["title_id"]?>">
			...
			<td class="tCenter"<?php if(!empty($v["Vote"]["review"])):?> title="<?php echo h($v["Vote"]["review"]) . " (" . mb_strlen($v["Vote"]["review"]) . ")"?>"<?php endif;?>><?php echo (!empty($v["Vote"]["review"])) ? "(" . mb_strlen($v["Vote"]["review"]) . ")" : " "?></td>
