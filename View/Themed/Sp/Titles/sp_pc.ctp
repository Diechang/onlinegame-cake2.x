...
//set blocks
$this->assign("title", "推奨PC（パソコン） | " . $titleWithStrs["Abbr"]);
...
$this->assign("description", $titleWithStrs["Sub"] . "の推奨PC(パソコン)です。ショップが推奨するゲームモデルPCで快適プレイ！" . $title["Titlesummary"]["pc_count"] . "件のPCが登録されています。");
//assigns
...
<section class="title-pc">
	...
	<div class="title-pc-pickup">
		<span class="ribbon">おすすめ</span>
		<?php echo $this->Common->adLinkText($pickup["Pc"], "pc", array("class" => "title"))?>
		...
		<div class="data">
			<table>
				...
				<tr>
					<!-- ショップ -->
					<td class="shop"><?php echo $pickup["Pcshop"]["ad_use"] ? $pickup["Pcshop"]["ad_text"] : $this->Html->link($pickup["Pcshop"]["shop_name"], $pickup["Pcshop"]["shop_url"], array("target" => "_blank"))?></td>
				</tr>
				<tr>
					<!-- 価格 -->
...
<section class="title-pcs title-pcs-<?php echo $pctype["Pctype"]["path"]?>">
			...
			<dd>
				...
				<div class="shop">
					<?php echo $this->Html->link("商品の購入・詳細はこちら",
								...
								array("class" => "button button-s button-block button-official", "target" => "_blank"))?>
				</div>
			</dd>
		<?php endforeach;?>
		</dl>
