...
<?php if(isset($title["Title"]["votable"]) && $title["Title"]["votable"]):?>
			<tr>
				...
				<td>
					...
					/ <a href="<?php echo $html->url(array("action" => "review" , "path" => $title["Title"]["url_str"] , "ext" => "html"))?>">レビュー：<?php echo $this->Common->countFormat($title["Titlesummary"]["vote_count_review"])?>件</a>
					/ <a href="<?php echo $html->url(array("action" => "rating" , "path" => $title["Title"]["url_str"] , "ext" => "html"))?>">評価：<?php echo $this->Common->countFormat($title["Titlesummary"]["vote_count_vote"])?>件</a>
				</td>
			</tr>
<?php endif;?>
		</table>
