...
<?php foreach($waits as $wait): ?>
	<li>
		<a href="<?php echo $this->Html->url(array("controller" => "titles", "action" => "index", "path" => $wait["Title"]["url_str"], "ext" => "html"));?>">
			<div class="thumb">
				<?php echo $this->Html->image($this->Common->thumbName($wait["Title"]["thumb_name"]),
					array("alt" => $this->Common->titleWithCase($wait["Title"]["title_official"], $wait["Title"]["title_read"])));?>
			</div>
