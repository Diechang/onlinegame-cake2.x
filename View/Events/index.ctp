...
<div class="content events">
	...
	<p class="icon_feed16"><?php echo $this->Html->link("新着イベント・キャンペーン情報をRSSで！" , "http://feeds.feedburner.com/dz-game/events")?></p>
	<p><?php echo $this->Paginator->counter(array("format" => "イベント・キャンペーン総数<span class=\"wBold\">%count%件中</span> %start%件目 ～ %end%件表示"))?></p>
	<p class="paging">
...
<?php foreach($events as $event):?>
		<li>
			<h3 class="<?php echo $this->Common->checkTerm($event["Event"]["start"] , $event["Event"]["end"])?>"><?php echo $this->Html->link($event["Event"]["name"] , array("controller" => "titles" , "action" => "event" , "path" => $event["Title"]["url_str"] , "eventid" => $event["Event"]["id"] , "ext" => "html"))?></h3>
			<div class="thumb">
